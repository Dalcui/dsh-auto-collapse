...
- **Keep the latest content visible while running**: while a turn is in progress, any second-level block with a running row is force-expanded (native tool card / reasoning rows stream live); on turn close it returns to the default collapsed state — i.e. the last piece of content is never folded while the conversation is active.
