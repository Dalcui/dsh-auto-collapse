...
`;
  var FoldController = class {
    ...
    reconcileBlock(block, segment, desiredHidden) {
      ...
      const keepRow = (row) => hasRunning && rowState(row) === "running" || row === keepLastRow;
  ...
  function deriveBlockInfo(rows, statusRows = [], excludeRunning = false, excludeRow = null) {
  ...
  function updateChip(chip, rows, expanded, statusRows = [], working = false, codeDescriptionMode = "always", keepLastRow = null) {
