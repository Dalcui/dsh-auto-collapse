...
var __dshcfBundle = (() => {
  ...
  function publishTurnMetrics(sessionId, turn, metrics) {
  ...
  function readTurnMetrics(sessionId, turn) {
    return metricsByTurn.get(`${sessionId}:${turn}`);
  }
  function readPreviousTurnLastInput(sessionId, turn) {
    let bestTurn = -1;
    let value;
    ...
    for (const key of order) {
      ...
      if (n.kind === "tool-call") {
        ...
        if (n.data && n.data.usage) {
          ...
          const stepTotal = (typeof u.inputTokens === "number" && isFinite(u.inputTokens) ? u.inputTokens : 0) + (typeof u.cacheReadTokens === "number" && isFinite(u.cacheReadTokens) ? u.cacheReadTokens : 0) + (typeof u.cacheWriteTokens === "number" && isFinite(u.cacheWriteTokens) ? u.cacheWriteTokens : 0);
...
`;
  var FoldController = class {
      ...
      for (const snapshot of segments) {
        ...
        if (attempts < 20 && (state.metrics === void 0 || !hasTokenMetrics(state.metrics) || state.metrics.cacheReadTokens === void 0 && state.metrics.cacheWriteTokens === void 0 || deltaPending)) {
          ...
          if (turnTail !== null) {
            ...
            const extracted = extractTurnMetrics(turnTail, keys.turn, keys.sessionId);
            if (extracted !== void 0) {
    ...
    buildLiveSummary(segment) {
      ...
      const published = keys.turn !== void 0 && keys.sessionId !== void 0 ? readTurnMetrics(keys.sessionId, keys.turn) : void 0;
      let liveDuration;
      if (published?.turnStartTime !== void 0) {
        liveDuration = Math.max(0, Date.now() - published.turnStartTime);
      ...
      if (keys.turn !== void 0 && keys.sessionId !== void 0 && published?.lastModelInputTokens !== void 0) {
        const prev = readPreviousTurnLastInput(keys.sessionId, keys.turn);
        if (prev !== void 0) liveMetrics.contextDelta = published.lastModelInputTokens - prev;
        else if (keys.turn === 1) liveMetrics.contextDelta = published.lastModelInputTokens;
      }
  ...
  function extractTurnMetrics(turnTail, turn, sessionId) {
    const text = turnTail.textContent ?? "";
    const metrics = {};
    if (turn !== void 0 && sessionId !== void 0) {
      const published = readTurnMetrics(sessionId, turn);
      if (published) {
    ...
    if (turn !== void 0 && sessionId !== void 0 && metrics.lastModelInputTokens !== void 0) {
      const prev = readPreviousTurnLastInput(sessionId, turn);
      if (prev !== void 0) metrics.contextDelta = metrics.lastModelInputTokens - prev;
      else if (turn === 1) metrics.contextDelta = metrics.lastModelInputTokens;
    }
  ...
  function segmentMetricsKeys(segment) {
    ...
    for (const el of candidates) {
      const inner = el.querySelector?.("[data-turn-tail], [data-dshcf-turn], [data-dshcf-session]");
      for (const src of [el, inner]) {
        ...
        if (turn !== void 0 && sessionId !== void 0) break;
      }
      if (turn !== void 0 && sessionId !== void 0) break;
    }
  ...
  function subToolsIn(row) {
    const names = [];
    for (const sub of row.querySelectorAll("[data-subcalls] [data-tool]")) {
  ...
  function deriveBlockInfo(rows, statusRows = []) {
  ...
  function updateChip(chip, rows, expanded, statusRows = []) {
    ...
    if (info.runningTool !== null) {
      ...
      if (collapsed && info.ptcDescription !== void 0 && info.ptcDescription !== "") {
        summaryText = countText === "" ? info.ptcDescription : countText + " \xB7 " + info.ptcDescription;
      }
  ...
  function parseSummaryFields(fields) {
    ...
    for (const raw of fields.split(",")) {
      ...
      specs.push(label !== void 0 && label !== "" ? { key, label } : { key });
    }
    return specs;
  }
  function buildMetricsSummary(duration, metrics, fields, running = false) {
    ...
    const fieldList = orderedFields.length > 0 ? orderedFields : ["duration", "toolCalls", "modelCalls", "inputTokens", "contextDelta", "outputTokens", "reasoningTokens", "cacheReadTokens", "cacheWriteTokens", "cacheHitRate", "timeToFirstToken", "tokensPerSecond"].map((key) => ({ key }));
  ...
  function renderMetricPart(field, duration, metrics, running = false) {
    ...
    const suffix = (value, z, e) => custom !== void 0 ? value + " " + custom : value + (zh ? z : e);
    switch (key) {
      case "duration": {
      ...
      case "timeToFirstToken": {
        ...
        if (custom !== void 0) return ttftValue(metrics.timeToFirstToken) + " " + custom;
        return formatTimeToFirstToken(metrics.timeToFirstToken);
      }
      case "tokensPerSecond":
        return metrics !== void 0 && metrics.tokensPerSecond !== void 0 && metrics.tokensPerSecond > 0 ? custom !== void 0 ? metrics.tokensPerSecond + " " + custom : metrics.tokensPerSecond + " tok/s" : null;
      default:
        return null;
    }
  }
