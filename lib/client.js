...
`;
  var FoldController = class {
    constructor() {
      ...
      /** 已成功收尾的回合边界（分批渲染补折叠用）。 */
      this.settledBoundaries = /* @__PURE__ */ new WeakSet();
      /** 被认领为"回合最终输出"的正文消息（分批渲染时可撤销：新消息
       * 挂载后它不再是最后一个 → 撤销认领并折叠）。 */
      this.finalStepHosts = /* @__PURE__ */ new WeakSet();
  ...
  function findNativeCommandSvg() {
    ...
    for (const drow of document.querySelectorAll("[data-chat-call-id] [data-disclosure-row]")) {
      if (drow.closest("[data-tool]") !== null) continue;
      for (const svg of drow.querySelectorAll("svg")) {
  ...
  function findSettledBoundary(flow, anchor, settled) {
