...
var __dshcfBundle = (() => {
  ...
  function publishTurnMetrics(turn, metrics) {
    if (metrics === null) {
  ...
  function readTurnMetrics(turn) {
    return metricsByTurn.get(turn);
  }
  function computeTurnMetrics(turn, order, nodes, turnTimings) {
  ...
  function TurnMetricsNodeView(props) {
    ...
    const metrics = useMemo(
      () => computeTurnMetrics(turn, order, nodes, turnTimings),
...
`;
  var FoldController = class {
    constructor(statusTextProvider2, summaryFieldsProvider2) {
      this.observer = null;
      this.raf = 0;
      this.timer = 0;
      this.disposed = false;
    ...
    pass() {
      ...
      for (const snapshot of segments) {
        ...
        if (attempts < 20 && (state.metrics === void 0 || !hasTokenMetrics(state.metrics) || state.metrics.cacheReadTokens === void 0 && state.metrics.cacheWriteTokens === void 0)) {
  ...
  function extractTurnMetrics(turnTail, turn) {
  ...
  function segmentTurnNumber(segment) {
    const candidates = [];
  ...
  function findTurnTail(segment) {
    ...
    if (segment.finalStep !== null) {
      ...
      while (el !== null) {
        if (el.matches('[data-chat-flow-kind="turn-tail"], [data-chat-flow-kind="turn-tail-timing"]')) return el;
  ...
  function buildMetricsSummary(duration, metrics, fields) {
    const parts = [];
    const orderedFields = fields ? Array.from(new Set(fields.split(",").map((f) => f.trim()).filter(Boolean))) : [];
    const fieldList = orderedFields.length > 0 ? orderedFields : ["duration", "toolCalls", "modelCalls", "inputTokens", "outputTokens", "reasoningTokens", "cacheReadTokens", "cacheWriteTokens", "cacheHitRate", "timeToFirstToken", "tokensPerSecond"];
  ...
  function renderMetricPart(field, duration, metrics) {
    switch (field) {
      case "duration":
        return duration !== void 0 ? formatDuration(duration) : null;
      case "toolCalls":
        return metrics !== void 0 && metrics.toolCalls !== void 0 && metrics.toolCalls > 0 ? metrics.toolCalls + (getLocale() === "zh" ? "\u6B21\u5DE5\u5177\u8C03\u7528" : " tool calls") : null;
      case "modelCalls":
        return metrics !== void 0 && metrics.modelCalls !== void 0 && metrics.modelCalls > 0 ? metrics.modelCalls + (getLocale() === "zh" ? "\u6B21\u6A21\u578B\u8C03\u7528" : " model calls") : null;
      case "inputTokens":
        return metrics !== void 0 && metrics.inputTokens !== void 0 && metrics.inputTokens > 0 ? formatTokensShort(metrics.inputTokens) + (getLocale() === "zh" ? "\u8F93\u5165" : " in") : null;
      case "outputTokens":
        return metrics !== void 0 && metrics.outputTokens !== void 0 && metrics.outputTokens > 0 ? formatTokensShort(metrics.outputTokens) + (getLocale() === "zh" ? "\u8F93\u51FA" : " out") : null;
      case "reasoningTokens":
        return metrics !== void 0 && metrics.reasoningTokens !== void 0 && metrics.reasoningTokens > 0 ? formatTokensShort(metrics.reasoningTokens) + (getLocale() === "zh" ? "\u63A8\u7406" : " rsn") : null;
      case "cacheReadTokens":
        return metrics !== void 0 && metrics.cacheReadTokens !== void 0 && metrics.cacheReadTokens > 0 ? formatTokensShort(metrics.cacheReadTokens) + (getLocale() === "zh" ? "\u7F13\u5B58\u547D\u4E2D" : " cache hit") : null;
      case "cacheWriteTokens":
        return metrics !== void 0 && metrics.cacheWriteTokens !== void 0 && metrics.cacheWriteTokens > 0 ? formatTokensShort(metrics.cacheWriteTokens) + (getLocale() === "zh" ? "\u7F13\u5B58\u5199\u5165" : " cache write") : null;
      case "cacheHitRate":
        return formatCacheHitRate(metrics);
      case "timeToFirstToken":
        return metrics !== void 0 && metrics.timeToFirstToken !== void 0 && metrics.timeToFirstToken > 0 ? formatTimeToFirstToken(metrics.timeToFirstToken) : null;
      case "tokensPerSecond":
        return metrics !== void 0 && metrics.tokensPerSecond !== void 0 && metrics.tokensPerSecond > 0 ? metrics.tokensPerSecond + " tok/s" : null;
      default:
        return null;
    }
  }
  ...
  function summaryFieldsProvider(scope) {
    return () => {
      if (scope === void 0) return "duration,toolCalls,inputTokens,outputTokens,cacheReadTokens,cacheHitRate";
      const snapshot = scope.getSnapshot();
      const value = snapshot.value;
...
`;
  ...
  function apply(ctx) {
    ctx.effect(() => {
      if (ctx.slots !== void 0) installTurnMetricsInjector(ctx);
