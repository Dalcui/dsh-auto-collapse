...
`;
  ...
  var DEFAULT_KEEP_LAST_BODY_STEPS2 = 1;
  ...
  function keepLastBodyStepsProvider(scope) {
    return () => {
      if (scope === void 0) return DEFAULT_KEEP_LAST_BODY_STEPS2;
      const snapshot = scope.getSnapshot();
      const value = snapshot.value;
      const raw = value?.keepLastBodySteps;
      const n = typeof raw === "number" && Number.isFinite(raw) ? Math.max(0, Math.floor(raw)) : DEFAULT_KEEP_LAST_BODY_STEPS2;
      return n;
    };
  }
  var CARD_CSS = `
...
`;
  ...
  function StatusTextCard(props) {
    ...
    const currentBodySteps = value?.keepLastBodySteps ?? DEFAULT_KEEP_LAST_BODY_STEPS2;
    const defaultBodySteps = base?.keepLastBodySteps ?? DEFAULT_KEEP_LAST_BODY_STEPS2;
      ...
      try {
        ...
        if (bodyStepsPending !== null) {
          if (bodyStepsPending.reset) await scope.unset("keepLastBodySteps");
            ...
            await scope.set("keepLastBodySteps", Number.isFinite(n) && n >= 0 ? Math.floor(n) : DEFAULT_KEEP_LAST_BODY_STEPS2);
          }
          setBodyStepsPending(null);
        }
      } catch {
