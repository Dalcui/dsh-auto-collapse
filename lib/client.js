...
var __dshcfBundle = (() => {
  ...
  function computeTurnMetrics(turn, order, nodes, turnTimings, nodeKey) {
    ...
    if (tu !== void 0 && tu !== null && turnTailSeg === targetSeg) {
      ...
      if (uncached !== void 0 && outputT !== void 0) {
...
`;
  var FoldController = class {
    constructor(statusTextProvider2, summaryFieldsProvider2, codeDescriptionProvider2, keepLastRowsProvider2) {
      this.observer = null;
      this.raf = 0;
      this.timer = 0;
      this.disposed = false;
  ...
  function renderMetricPart(field, duration, metrics, running = false) {
    ...
    switch (key) {
      ...
      case "tokensPerSecond":
        return metrics !== void 0 && metrics.tokensPerSecond !== void 0 && metrics.tokensPerSecond > 0 ? custom !== void 0 ? formatTokensPerSecond(metrics.tokensPerSecond) + " " + custom : formatTokensPerSecond(metrics.tokensPerSecond) + " tok/s" : null;
      default:
        return null;
    }
  }
  ...
  var DEFAULT_OWN_ID = "dsh-auto-collapse";
  var DEFAULT_ENDPOINT = "/dsh-auto-collapse/roster";
  ...
  function keepLastRowsProvider(scope) {
    return () => {
      if (scope === void 0) return DEFAULT_KEEP_LAST_ROWS2;
      const snapshot = scope.getSnapshot();
      const value = snapshot.value;
      const raw = value?.keepLastRows;
      const n = typeof raw === "number" && Number.isFinite(raw) ? Math.max(0, Math.floor(raw)) : DEFAULT_KEEP_LAST_ROWS2;
      return n;
    };
  }
  var CARD_CSS = `
...
`;
  ...
  function StatusTextCard(props) {
    ...
    const currentRows = value?.keepLastRows ?? DEFAULT_KEEP_LAST_ROWS2;
    const defaultRows = base?.keepLastRows ?? DEFAULT_KEEP_LAST_ROWS2;
      ...
      try {
        ...
        if (rowsPending !== null) {
          if (rowsPending.reset) await scope.unset("keepLastRows");
            ...
            await scope.set("keepLastRows", Number.isFinite(n) && n >= 0 ? Math.floor(n) : DEFAULT_KEEP_LAST_ROWS2);
          }
          setRowsPending(null);
        }
      } catch {
