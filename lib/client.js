...
var __dshcfBundle = (() => {
  ...
  __export(client_exports, {
    apply: () => apply,
    createRemoteConfigStore: () => createRemoteConfigStore,
    inject: () => inject,
    installRosterWatchdog: () => installRosterWatchdog,
    name: () => name,
    rosterSignature: () => rosterSignature,
    sanitizeRemoteConfig: () => sanitizeRemoteConfig,
    shouldReloadRoster: () => shouldReloadRoster,
    wrapScopeWithRemote: () => wrapScopeWithRemote
  });
  ...
  var lastSegBySession = /* @__PURE__ */ new Map();
  ...
  function syncLastSegIndex(sessionId, turn, segOrdinal, removed) {
    ...
    if (!removed) {
      if (current === void 0 || segOrdinal > current) byTurn.set(turn, segOrdinal);
      return;
    }
  ...
  function readPreviousTurnLastInput(sessionId, turn, segOrdinal = 0) {
    ...
    if (byTurn === void 0) return void 0;
    let bestTurn = -1;
    let bestSeg = -1;
  ...
  var TURN_SCOPE_SEG = -1;
  function newGroupAccumulator() {
    return {
      toolCalls: 0,
      modelCalls: 0,
      input: 0,
  ...
  function accumulateNode(n, acc) {
    if (typeof n.time === "number" && isFinite(n.time)) {
      if (acc.firstEventTime === void 0 || n.time < acc.firstEventTime) acc.firstEventTime = n.time;
    }
    if (n.kind === "tool-call") {
      acc.toolCalls++;
      acc.hasContent = true;
    ...
    if (n.kind === "model-retry") {
      ...
      if (Array.isArray(attempts)) {
        acc.modelCalls += attempts.filter((a) => a !== null && typeof a === "object" && a.retryState === "started").length;
      }
      return;
    }
    if (n.kind !== "assistant-step") return;
    if (!n.data || n.data.finalNode === void 0) return;
    acc.modelCalls++;
    acc.hasContent = true;
    const stepTiming = n.data.finalNode?.timing ?? n.data.timing ?? n.timing;
    const firstToken = stepTiming !== null && typeof stepTiming === "object" && typeof stepTiming.firstTokenTime === "number" && isFinite(stepTiming.firstTokenTime) ? stepTiming.firstTokenTime : void 0;
    if (firstToken !== void 0 && (acc.firstTokenTime === void 0 || firstToken < acc.firstTokenTime)) {
      acc.firstTokenTime = firstToken;
    }
    const stepStart = typeof stepTiming?.stepStartTime === "number" && isFinite(stepTiming.stepStartTime) ? stepTiming.stepStartTime : void 0;
    if (stepStart !== void 0 && (acc.firstStepStart === void 0 || stepStart < acc.firstStepStart)) {
    ...
    if (stepPrompt !== void 0) acc.input += stepPrompt;
    ...
    if (settled && stepTiming !== null && typeof stepTiming === "object") {
      ...
      if (firstToken !== void 0 && typeof completed === "number" && isFinite(completed) && typeof stepOut === "number" && isFinite(stepOut) && stepOut >= 0 && completed > firstToken) {
    ...
    if (stepPrompt !== void 0 && stepPrompt > 0) acc.lastModelInput = stepPrompt;
  }
  function finalizeGroupMetrics(acc, durationMs, turnStartTime, turnEndTime, billed) {
    let input = acc.input;
    ...
    if (tokensPerSecond === void 0 && acc.liveDecodeMs > 0) {
      tokensPerSecond = acc.liveOutputTokens / (acc.liveDecodeMs / 1e3);
    }
    return {
      durationMs,
      toolCalls: acc.toolCalls > 0 ? acc.toolCalls : void 0,
      modelCalls: acc.modelCalls > 0 ? acc.modelCalls : void 0,
  ...
  function buildTurnGroupMetrics(turn, order, nodes, turnTimings) {
    if (turn === void 0 || !order || !nodes) return /* @__PURE__ */ new Map();
    return buildGroupsFromIndex(turn, getTurnSegIndex(order, nodes), nodes, turnTimings);
  }
  function buildGroupsFromIndex(turn, segIndex, nodes, turnTimings) {
    ...
    const segs = [...groups.keys()].sort((a, b) => a - b).filter((seg) => groups.get(seg).hasContent);
    const groupCount = segs.length;
    let turnAcc;
    if (groupCount <= 1) {
      turnAcc = segs.length === 1 ? groups.get(segs[0]) : newGroupAccumulator();
    } else {
      ...
      for (const seg of segs) {
        ...
        if (g.lastModelInput !== void 0) turnAcc.lastModelInput = g.lastModelInput;
      }
    }
    ...
    for (let index = 0; index < segs.length; index++) {
      const acc = groups.get(segs[index]);
      ...
      if (!coversTurn) {
        ...
        groupDurationMs = start !== void 0 && end !== void 0 ? Math.max(0, end - start) : void 0;
      }
      result.set(segs[index], finalizeGroupMetrics(acc, groupDurationMs, turnStartTime, turnEndTime, {
        usage: coversTurn ? turnTailUsage : void 0,
        tokensPerSecond: coversTurn ? turnTailTps : void 0,
        // 首 token 时延是回合首次模型调用的时延，归属持有首个分组的那个分组。
        timeToFirstToken: index === 0 ? turnTailTtft : void 0
      }));
    }
    if (groupCount <= 1 && segs.length === 1) {
      result.set(TURN_SCOPE_SEG, result.get(segs[0]));
    } else {
  ...
  function computeTurnMetrics(turn, order, nodes, turnTimings, nodeKey) {
    ...
    const targetSeg = nodeKey === void 0 ? 0 : segIndex.segOf.get(nodeKey) ?? 0;
  ...
  function cachedTurnMetrics(sessionId, turn, segOrdinal, order, nodes, turnTimings, nodeKey) {
    ...
    if (nodes === void 0 || typeof nodes.values !== "function") {
      if (order === void 0) return computeTurnMetrics(turn, order, nodes, turnTimings, nodeKey);
  ...
  function cachedSegOrdinal(nodeKey, order, nodes) {
  ...
  var turnSegIndexCache = /* @__PURE__ */ new WeakMap();
  function buildTurnSegIndex(order, nodes) {
    const segOf = /* @__PURE__ */ new Map();
    const keysByTurn = /* @__PURE__ */ new Map();
    let seg = 0;
    let currentTurn;
    for (const key of order) {
      ...
      if (loc !== void 0 && loc !== null && (loc.kind === "turn" || loc.kind === "step") && loc.turn) {
      ...
      if (n.kind === "steering") {
        ...
        segOf.set(key, seg);
        if (loc !== void 0 && loc !== null && (loc.kind === "turn" || loc.kind === "step") && loc.turn && currentTurn !== void 0) {
          let bucket = keysByTurn.get(currentTurn);
          if (bucket === void 0) {
            bucket = [];
            keysByTurn.set(currentTurn, bucket);
    ...
    try {
      ...
      for (const e of entries) {
        if (e && e.options && e.options.key === "assistant-step") {
          ...
          if (lowest === void 0 || p < lowest) lowest = p;
        }
      }
      if (lowest !== void 0) priority = Math.min(-1, lowest - 1);
    } catch {
    }
    try {
  ...
  function TurnMetricsNodeView(props) {
    ...
    const segOrdinal = useMemo(
      () => cachedSegOrdinal(nodeKey, order, nodes),
      [nodeKey, order, nodes]
    );
    ...
    const turnScopeMetrics = useMemo(
      () => nativeRowFoldable ? cachedTurnMetrics(sessionId, turn, TURN_SCOPE_SEG, order, nodes, turnTimings, nodeKey) : null,
    ...
    const isTurnScopeHost = scopeSpec !== void 0 && scopeSpec !== null && typeof nodeStep === "number" && scopeSpec.answerStep === nodeStep;
    ...
    useEffect(() => {
      ...
      if (turnScopeMetrics !== null && turnScopeMetrics !== void 0) {
      ...
      if (el && typeof el.setAttribute === "function") {
        ...
        if (isTurnScopeHost && turnScopeMetrics !== null && turnScopeMetrics !== void 0) {
  ...
  function sameRemoteConfig(a, b) {
    if (a === b) return true;
    if (a === null || b === null) return false;
    return a.statusText === b.statusText && a.summaryFields === b.summaryFields && a.codeDescription === b.codeDescription && a.keepLastRows === b.keepLastRows && a.keepLastBodySteps === b.keepLastBodySteps;
  }
  ...
  function wrapScopeWithRemote(scope, remote) {
    const derive = () => {
      const inner = scope?.getSnapshot();
      if (inner !== void 0 && inner.status === "ready") return inner;
    ...
    const writableInner = () => scope !== void 0 && scope.getSnapshot().status === "ready";
    return {
      getSnapshot: derive,
      subscribe(listener) {
    ...
    return React.createElement("li", { className: cardClass }, [
      ...
      open ? React.createElement("div", { className: "dshcf-settings-body" }, [
        ...
        React.createElement("div", { className: "dshcf-settings-field" }, [
          ...
          React.createElement("p", { className: "dshcf-settings-hint" }, "\u8FDB\u884C\u4E2D\u7684\u8F6E\u6B21\u4E2D\uFF0C\u6700\u540E N \u4E2A\u7CFB\u7EDF\u63D0\u793A\u884C\uFF08\u601D\u8003 / \u5DE5\u5177 / \u4E0A\u4E0B\u6587 / \u91CD\u8BD5\xB7\u5931\u8D25\xB7\u8F93\u51FA\u4E0A\u9650\u7B49\u72B6\u6001\u63D0\u793A\u884C\uFF09\u4E0D\u6536\u5165\u6298\u53E0\uFF0C\u4FDD\u7559\u539F\u751F\u663E\u793A\uFF1B\u9ED8\u8BA4 3\uFF0C\u586B 0 \u8868\u793A\u4E0D\u4FDD\u7559\u4EFB\u4F55\u7CFB\u7EDF\u884C\uFF08\u542B\u6B63\u5728\u8FD0\u884C\u7684\u884C\uFF0C\u5168\u90E8\u6298\u53E0\uFF09\u3002")
        ]),
...
/* \u6302\u8FDB DSH \u539F\u751F turn-process disclosure \u884C\u7684\u6307\u6807\u6458\u8981\uFF08compact \u6A21\u5F0F\u534F\u540C\uFF09\u3002
   ...
   \u4E4B\u540E\u5168\u90E8\u5269\u4F59\u5BBD\u5EA6\uFF0C\u8D85\u5BBD\u6587\u672C\u5728 span \u5185\u6362\u884C\uFF08\u5206\u9694\u7B26\u300C\xB7\u300D\u5904\u81EA\u7136\u65AD\u884C\uFF09\uFF0C
   \u884C\u9AD8\u968F\u884C\u6570\u81EA\u9002\u5E94\uFF0C\u4E0D\u518D\u4E22\u5931\u5C3E\u90E8\u6307\u6807\u3002 */
...
`;
  var FoldController = class {
    ...
    pass() {
      ...
      if (nativeTurnButtons === void 0) {
        nativeTurnButtons = [...flow.querySelectorAll("[data-turn-process]")];
        nativeTurnButtonsCache.set(flow, nativeTurnButtons);
      }
      const nativeTurns = /* @__PURE__ */ new Set();
      const nativeOpenTurns = /* @__PURE__ */ new Set();
      ...
      const segmentKeys = /* @__PURE__ */ new Map();
      ...
      const nativeOpenSegments = /* @__PURE__ */ new Set();
      for (const segment of segments) {
        if (!nativeManaged.has(segment.key)) continue;
        const keys = segmentKeys.get(segment.key);
        if (keys !== void 0 && keys.turn !== void 0 && nativeOpenTurns.has(String(keys.turn))) nativeOpenSegments.add(segment.key);
      }
      const groupScopeOf = /* @__PURE__ */ new Map();
      const coversTurnOf = /* @__PURE__ */ new Map();
      const contentSegsOfTurn = /* @__PURE__ */ new Map();
      const publishedSegOfTurn = /* @__PURE__ */ new Map();
      for (const segment of segments) {
        ...
        if (published !== void 0) {
          ...
          if (prev === void 0 || published > prev) publishedSegOfTurn.set(id, published);
        }
      }
      for (const segment of segments) {
        const keys = segmentKeys.get(segment.key);
        ...
        const multi = published !== void 0 ? published > 0 : (contentSegsOfTurn.get(id) ?? 1) > 1;
      ...
      for (const snapshot of segments) {
        ...
        const parsed = turnTail === null || !coversTurn ? void 0 : parseTurnDuration(turnTail);
    ...
    reconcileBlock(block, segment, desiredHidden, keepTrailing, keepLastRows, nativeCollapsed, nativeManaged) {
      ...
      const levelCollapsed = !nativeManaged && state !== void 0 && !state.expanded;
      ...
      const hiddenCount = block.rows.filter((row) => !keepRow(row)).length + block.statusRows.filter((status) => !keepRow(status)).length;
  ...
  function extractTurnMetrics(turnTail, turn, sessionId, segOrdinal = 0, coversTurn = true) {
    ...
    if (turn !== void 0) {
      try {
        ...
        const hosts = cachedHosts ?? document.querySelectorAll("[data-dshcf-turn-metrics]");
        const turnStr = String(turn);
        const segStr = String(segOrdinal);
    ...
    if (metrics.durationMs === void 0 && coversTurn) {
    ...
    if (metrics.tokensPerSecond === void 0 && coversTurn && tpsMatch !== null) metrics.tokensPerSecond = Number(tpsMatch[1]);
    ...
    if (coversTurn && (metrics.inputTokens === void 0 || metrics.outputTokens === void 0 || metrics.reasoningTokens === void 0)) {
      const candidates = [];
    ...
    if (metrics.timeToFirstToken === void 0 && coversTurn) {
  ...
  var publishedSegCache = /* @__PURE__ */ new WeakMap();
  function contentNodeCount(segment) {
  ...
  function segmentMetricsKeys(segment) {
    ...
    for (const el of candidates) {
      if (turn === void 0) {
        turn = parseTurn(el.getAttribute("data-chat-turn")) ?? parseTurn(el.getAttribute("data-turn-tail")) ?? parseTurn(el.getAttribute("data-dshcf-turn"));
      }
      if (sessionId === void 0) {
    ...
    if (sessionId === void 0 && typeof document !== "undefined") {
      const flow = (segment.boundary ?? segment.finalStep ?? segment.blocks[0]?.host)?.closest?.("[data-chat-flow]") ?? null;
      if (flow !== null && flow !== void 0) {
        ...
        if (cached === void 0) {
          ...
          cached = ds !== null && ds !== void 0 && ds !== "" ? ds : "";
  ...
  function publishedSegOfSegment(segment) {
    const flow = segment.finalStep?.closest?.("[data-chat-flow]") ?? segment.blocks[0]?.host?.closest?.("[data-chat-flow]") ?? null;
    ...
    for (const step of segment.bodySteps) {
      ...
      if (hit === void 0 || hit.v !== version) {
        const el = step.querySelector?.("[data-dshcf-seg]") ?? null;
        const raw = el === null ? null : el.getAttribute("data-dshcf-seg");
        const parsed = raw === null || raw === "" ? void 0 : Number(raw);
        hit = { v: version, seg: parsed !== void 0 && Number.isFinite(parsed) && parsed >= 0 ? parsed : void 0 };
        publishedSegCache.set(step, hit);
      }
      if (hit.seg !== void 0 && (best === void 0 || hit.seg > best)) best = hit.seg;
    }
    return best;
  }
  ...
  function flowOrderIndexOf(el, itemIndex) {
    let cur = el;
    while (cur !== null && !itemIndex.has(cur)) cur = cur.parentElement;
    return cur === null ? Number.MAX_SAFE_INTEGER : itemIndex.get(cur);
  }
  ...
  function buildSegments(flow, blocks, hasBody, keepLastBodySteps = DEFAULT_KEEP_LAST_BODY_STEPS) {
    ...
    const append = (end, boundary, closed, turnNumber2) => {
      ...
      const kindOf = (el) => {
        ...
        if (kind === void 0 || kind.v !== structVersion) {
      ...
      if (!closed && !terminated) {
        ...
        if (candidates.length > 1) {
          sysRowOrder.push(...candidates.map((el, order) => ({ el, pos: flowOrderIndexOf(el, itemIndex), order })).sort((a, b) => a.pos - b.pos || a.order - b.order).map((entry) => entry.el));
        } else {
  ...
  function findBlocks(flow, hasBody) {
    ...
    for (const el of children) {
      ...
      if (collect === void 0 || collect.version !== structVersion) {
        collect = {
  ...
  function deriveBlockInfo(rows, statusRows = [], excludeRunning = false, excludeRows = null) {
    ...
    return {
      ...
      retryCount: statusRows.filter((el) => (el.getAttribute("data-chat-flow-kind") ?? "") === "model-retry" && !(excludeRows !== null && excludeRows.has(el))).length
    };
  }
  ...
  var timeStartIndexCache = /* @__PURE__ */ new WeakMap();
  var nativeTurnButtonsCache = /* @__PURE__ */ new WeakMap();
  var metricsHostListCache = /* @__PURE__ */ new WeakMap();
  var flowSessionCache = /* @__PURE__ */ new WeakMap();
  var rowCollectCache = /* @__PURE__ */ new WeakMap();
  var kindCache = /* @__PURE__ */ new WeakMap();
  var flowStructureVersion = /* @__PURE__ */ new WeakMap();
  function nextFlowStructureVersion(flow) {
    const next = (flowStructureVersion.get(flow) ?? 0) + 1;
    flowStructureVersion.set(flow, next);
    return next;
  }
  function metricsHostsOf(root) {
    let list = metricsHostListCache.get(root);
    if (list === void 0) {
      if (typeof root.querySelectorAll !== "function") return null;
      list = [...root.querySelectorAll("[data-dshcf-turn-metrics]")];
      metricsHostListCache.set(root, list);
    }
    return list;
  }
  function invalidateRowCollectCache(flow) {
    nextFlowStructureVersion(flow);
  }
  function invalidateFlowIndexes(flow) {
  ...
  function findTurnStart(boundary) {
    ...
    if (starts === void 0) {
      starts = [...flow.querySelectorAll('[class*="timeStart"]')];
  ...
  function apply(ctx) {
    ctx.effect(() => {
      ...
      const offSettings = ctx.slots === void 0 || rawScope === void 0 ? void 0 : setupSettingsCard(ctx, scope);
      let offWatchdog = () => {
      };
      try {
