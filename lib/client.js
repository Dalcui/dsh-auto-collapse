...
var __dshcfBundle = (() => {
    ...
    if (nodeKey !== void 0) {
      ...
      for (const key of order) {
        ...
        if (n && n.location && (n.location.kind === "turn" || n.location.kind === "step") && n.location.turn) {
          ...
          if (currentTurn2 !== void 0 && t !== currentTurn2) seg2 = 0;
          currentTurn2 = t;
        }
        if (key === nodeKey) {
          targetSeg = seg2;
  ...
  function TurnMetricsNodeView(props) {
    ...
    const metrics = useMemo(
      () => computeTurnMetrics(turn, order, nodes, turnTimings, nodeKey),
...
`;
  var FoldController = class {
    ...
    buildLiveSummary(segment) {
      ...
      const published = keys.turn !== void 0 && keys.sessionId !== void 0 ? readTurnMetrics(keys.sessionId, keys.turn, seg) : void 0;
      let liveDuration;
      if (published?.turnStartTime !== void 0 && seg === 0) {
    ...
    reconcileBlock(block, segment, desiredHidden) {
      ...
      const containerHasRunning = (container) => hasRunning && block.rows.some((row) => rowState(row) === "running" && isDescendantOf(row, container));
      for (const container of block.containers) {
        if (expanded || containerHasRunning(container)) this.restoreElement(container, animate);
        else {
      ...
      for (const row of block.rows) {
        if (expanded || hasRunning && rowState(row) === "running") this.restoreElement(row, animate);
        else {
  ...
  function deriveBlockInfo(rows, statusRows = [], excludeRunning = false) {
    ...
    const countedPairs = excludeRunning ? pairs.filter((p) => p.info.state !== "running") : pairs;
  ...
  function updateChip(chip, rows, expanded, statusRows = [], working = false) {
