...
`;
  var FoldController = class {
    ...
    pass() {
      ...
      for (const snapshot of segments) {
        ...
        if ((state.metrics === void 0 || !hasTokenMetrics(state.metrics)) && attempts < 20) {
  ...
  function buildMetricsSummary(duration, metrics, fields) {
    const parts = [];
    const fieldSet = fields ? new Set(fields.split(",").map((f) => f.trim()).filter(Boolean)) : /* @__PURE__ */ new Set();
    if (duration !== void 0 && (fieldSet.size === 0 || fieldSet.has("duration"))) {
      parts.push(formatDuration(duration));
    }
    if (metrics !== void 0) {
      if (metrics.toolCalls !== void 0 && metrics.toolCalls > 0 && (fieldSet.size === 0 || fieldSet.has("toolCalls"))) {
        parts.push(metrics.toolCalls + (getLocale() === "zh" ? "\u6B21\u5DE5\u5177\u8C03\u7528" : " tool calls"));
      }
      if (metrics.inputTokens !== void 0 && metrics.inputTokens > 0 && (fieldSet.size === 0 || fieldSet.has("inputTokens"))) {
        parts.push(formatTokensShort(metrics.inputTokens) + (getLocale() === "zh" ? "\u8F93\u5165" : " in"));
      }
      if (metrics.outputTokens !== void 0 && metrics.outputTokens > 0 && (fieldSet.size === 0 || fieldSet.has("outputTokens"))) {
        parts.push(formatTokensShort(metrics.outputTokens) + (getLocale() === "zh" ? "\u8F93\u51FA" : " out"));
      }
      if (metrics.reasoningTokens !== void 0 && metrics.reasoningTokens > 0 && (fieldSet.size === 0 || fieldSet.has("reasoningTokens"))) {
        parts.push(formatTokensShort(metrics.reasoningTokens) + (getLocale() === "zh" ? "\u63A8\u7406" : " rsn"));
      }
      if (metrics.modelCalls !== void 0 && metrics.modelCalls > 0 && (fieldSet.size === 0 || fieldSet.has("modelCalls"))) {
        parts.push(metrics.modelCalls + (getLocale() === "zh" ? "\u6B21\u6A21\u578B\u8C03\u7528" : " model calls"));
      }
      if (metrics.tokensPerSecond !== void 0 && metrics.tokensPerSecond > 0 && (fieldSet.size === 0 || fieldSet.has("tokensPerSecond"))) {
        parts.push(metrics.tokensPerSecond + " tok/s");
      }
      if (metrics.timeToFirstToken !== void 0 && metrics.timeToFirstToken > 0 && (fieldSet.size === 0 || fieldSet.has("timeToFirstToken"))) {
        const secs = metrics.timeToFirstToken >= 1e4 ? Math.round(metrics.timeToFirstToken / 1e3) : Math.round(metrics.timeToFirstToken / 100) / 10;
  ...
  function summaryFieldsProvider(scope) {
    return () => {
      if (scope === void 0) return "duration,toolCalls,inputTokens,outputTokens";
      const snapshot = scope.getSnapshot();
      const value = snapshot.value;
