...
- **二级 chip 分层粒度计数**：完成态 chip 收起时展示 `N 段思考 · M 次工具调用`（上下文块为 `N 次上下文注入`），有失败时追加浅红 `K 个失败`。对齐 dsh-turn-fold 的 activityGroup。
...
- **可配置指标字段**：在 设置 → 插件 → 插件配置 的"摘要栏指标"输入框中，用逗号分隔字段名控制显示哪些指标；默认显示耗时 + 工具调用 + 输入/输出 tokens + 缓存命中数 + 缓存命中率。
