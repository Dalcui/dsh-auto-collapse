...
- **回合级状态行随段折叠**：DSH 原生重试链（`model-retry`，"已重试模型请求…"）、终态失败（`turn-error`）、达到输出上限（`turn-max-tokens`）等状态装饰行都是 flow 直接子级、携带文本但非 assistant-step；随一级折叠隐藏、展开恢复，不残留可见、不打断工具组合并。工作中（未闭合）保持可见。
